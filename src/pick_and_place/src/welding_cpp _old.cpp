...
int main(int argc, char** argv)
{
  ...
  std::string model_name="welding_line";
  ...
  if (ros::service::call("/gazebo/get_model_state", get_model_state))
    {    // Return the pose values as a tuple
      ...
      x=get_model_state.response.pose.position.x;
      y=get_model_state.response.pose.position.y;
      z=get_model_state.response.pose.position.z;
           // Display the loaded values
      std::cout << model_name << " position: "
                << x << ", "
                << y << ", "
                << z << ", "
                << 2*std::atan2(get_model_state.response.pose.orientation.z,get_model_state.response.pose.orientation.w) <<std::endl;}
  else
  ...
  pickPose(group_arm,"down", start_x, start_y, z+0.20 );
  hoverPose(group_arm, end_x, end_y, z+0.14);
  ros::Duration(2.0).sleep();  // Delay for 2 seconds
  pickPose(group_arm, "up",end_x, y, z+0.20);
  initPose(group_arm);
  ROS_WARN("round1 end");
