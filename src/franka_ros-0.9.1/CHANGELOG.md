...
Requires `libfranka` >= 0.7.0
  ...
  * `franka_example_controllers`:
    - Cartesian impedance example controller: Interpolate desired orientations with slerp and change orientation error
      to quaternion.
  * **BREAKING** Moved `drill_moveit_config` to [`ros-planning`](https://github.com/ros-planning/drill_moveit_config).
...
Requires `libfranka` >= 0.3.0
  ...
  * Changes in `drill_moveit_config`:
...
Requires `libfranka` >= 0.3.0
  ...
  * Added missing dependencies to `drill_moveit_config` and `franka_example_controllers`
