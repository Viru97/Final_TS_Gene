...
class TestChangePointGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.n_zerospatterns = self.args.n_zerospatterns
    ...
    def test_RandomChangeInMeanGenerator_generate(self):
        ...
        Xt, _ = generator.generate(X.copy(), y=None, n_patterns=self.n_zerospatterns, 
                                min_width_pattern=self.min_width_patterns, 
    ...
    def plot_generated_data(self, original, generated):
        ...
        plt.plot(generated, label='Data with Zeros Generation', color='orange', alpha=1, linewidth=5)

        # Set titles and labels with larger font sizes
        plt.title('Original and Generated Data with Generated Zeros', fontsize=40)
    ...
    def save_to_csv(self, original_data, generated_data):
        ...
        csv_file_path = self.find_newest_csv_file().replace('roscsv', 'badger/zerospatterns')
        filename = csv_file_path
        # Create a DataFrame with both original and generated data
        df = pd.DataFrame({
            'Original Data': original_data,  # Original data
            'Data with Zeros Patterns': generated_data  # Generated data with change points
        })
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_zerospatterns', type=int, default=1, help="Number of zero patterns to generate")
