...
class ProgressPage(QWidget):    
    ...
    def monitor_logs(self):
        """
        Subscribes to ROS log messages and updates the progress bar.
        """
    ...
    def log_callback(self, data):
        ...
        if task_type == 2:
            if "round1 end" in data.msg:  # Check if the log message contains 'round end'
                self.update_progress_bar(33)  # Example progress value for round end
            if "round2 end" in data.msg:  # Check if the log message contains 'round end'
                self.update_progress_bar(66)  # Example progress value for round end
            if "round3 end" in data.msg:  # Check if the log message contains 'round end'
                self.update_progress_bar(100)  # Example progress value for round end

        if task_type == 1:
    # elif isinstance(current_widget, Welding_GeneratorPage):
            if "EXECUTION IS DONE" in data.msg:  # Check for execution completion
                count= count+1
                self.update_progress_bar(14*count)  # Example progress value
            if "Round end" in data.msg:
                self.update_progress_bar(100)  # Progress completion for round end
...
class DrillingExecutionPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.button_start.setIcon(QIcon('/home/baua/Final_TS_Gene/src/gui/logos/play.png'))
...
class Gui(QMainWindow):
    ...
    def go_to_progress_page(self):
        ...
        # self.pages['progress'].start_progress()
    ...
    def start_drilling_generator(self):
        ...
        for i in range(run_value):
            self.pages['progress'].start_progress()     
            self.pages['progress'].label.setText(f'Cycle: {i+1}/{run_value}')     
            ...
            try:
                ...
                self.pages['progress'].thread.join(timeout=1)  # Wait for the ROS thread to finish
    ...
    def generator_plot(self):
            ...
            self.convert_bag_to_csv(bag_file_path, csv_file_path, '/record')
            time.sleep(2)
            self.gene_plot = subprocess.Popen(['rosrun', 'panda_gazebo', 'plot.py', '--joint_state_column', str(joint_index.data)])
    ...
    def bias_fault_generator(self):
        ...
        try:
            ...
            self.start_time = rospy.get_time() + round(random.uniform(15, 30))
    ...
    def random_gene_badger_fault_generator(self):
        ...
        self.badger_numbers = random.randint(5, 10)
        self.badger_min = random.randint(10, 50)
        self.badger_max = random.randint(100, 500)
    ...
    def drift_badger_fault_generator(self):
        ...
        self.badger_numbers = random.randint(100, 1000)
        self.badger_min = random.uniform(-1, 0)
        self.badger_max = random.uniform(0, 1)
        self.pages['badger_page'].label_2.setText(f"\n\nFault Number: {self.badger_numbers}")
        self.pages['badger_page'].label_min.setText(f"\n\nFault Min: {self.badger_min}")
        self.pages['badger_page'].label_max.setText(f"\n\nFault Max: {self.badger_max}")
        return self.badger_numbers, self.badger_min, self.badger_max
        ...
        if self.badger_selected_option is not None and self.badger_numbers is not None and self.badger_joint_number is not None:
            ...
            if self.badger_selected_option == 'ZerosPattern':
                ...
                self.badger_process = subprocess.Popen(['python3', 'src/badgers-main/tests/generators/time_series/test_zerosger.py',
                                                        '--column', 'field.normal_joint_states.position' + str(self.badger_joint_number),
                ...
                self.badger_process = subprocess.Popen(['python3', 'src/badgers-main/tests/generators/time_series/test_changepoints.py',
                                                        '--column', 'field.normal_joint_states.position' + str(self.badger_joint_number),
                                                        '--n_changepoints', str(self.badger_numbers),
                                                        '--min_change', str(self.badger_min), 
                                                        '--max_change',str(self.badger_max)])  
            elif self.badger_selected_option == 'MissingPoints':   
                self.badger_joint_number = self.badger_fault_location_options()
...
if __name__ == "__main__":
    ...
    pub_index = rospy.Publisher('fault_index', Int32, queue_size=100, latch=True)
    pub_duration = rospy.Publisher('fault_duration', Float32, queue_size=100, latch=True)
    pub_amplitude = rospy.Publisher('fault_amplitude', Float32, queue_size=100, latch=True)
    pub_time = rospy.Publisher('fault_time', Float32, queue_size=100, latch=True)
