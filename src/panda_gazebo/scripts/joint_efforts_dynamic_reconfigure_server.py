#!/usr/bin/env python3
"""Small node that publishes Panda arm joint efforts and gripper width commands."""
...
class JointEffortsDynamicReconfigureServer(Node):
    """Node that publishes joint effort commands and optional gripper commands."""

    def __init__(self):
        super().__init__("joint_efforts_reconfig_server")
        self.get_logger().info("Starting ROS2 joint efforts helper node")
    ...
    def _initialize_from_joint_states(self):
        ...
        if not joint_states.effort:
            self.get_logger().warn("Received empty joint state efforts")
            return

        msg = Float64MultiArray()
        msg.data = list(joint_states.effort[:7])
        self.arm_pub.publish(msg)
        self._initialized = True
        self.get_logger().info("Initial joint effort command published")

    def publish_joint_efforts(self, efforts):
        msg = Float64MultiArray()
        msg.data = list(efforts[:7])
        self.arm_pub.publish(msg)
...
def main(args=None):
    ...
    node = JointEffortsDynamicReconfigureServer()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
